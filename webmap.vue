  ...
  <div id="web-header">
    <el-row >
      ...
      <el-button  @click="click()" id="bt_cd"></el-button>
          <el-select  v-model="value"   @change="changeSelect()" id="select_qx" style="width:199px;height:58px" >
            <el-option
...
<el-button @click="change1" id="bt_map"></el-button>
  ...
  <div id="web-main">
        ...
        <div id="bt_group">
          <div id="bt_data">
            <el-radio-group v-model="radio" @change="show_DataWindow">
              <el-radio-button label="基础数据"></el-radio-button>
              <el-radio-button label="运行数据"></el-radio-button>
              <!-- <el-radio-button label="其他数据"></el-radio-button> -->
            </el-radio-group>
          </div>
          <div id="bt_detail">
            ...
            <el-select  id="select1" v-model="value1" filterable  placeholder="搜索设备"  clearable   remote    default-first-option  :loading="loading"
              ...
              <el-select id="select2" v-model="value2" filterable  placeholder="搜索线路"  clearable   remote    default-first-option  :loading="loading2"
        ...
        <div id="map_control">
          ...
          <el-checkbox-button label="箱式变压器" style="width:70px">箱变</el-checkbox-button>
          <el-checkbox-button label="配电室" style="width:70px">配电室</el-checkbox-button>
          <el-checkbox-button label="电缆分接箱" style="width:70px">电分</el-checkbox-button>
  ...
  <!-- 配变数据配置 -->
        <div id="properties">
          <div id="properPB"  style="display:none">
             <div class="biaoqian">
              <div class="canshu"> 设备名称：{{pb.name}} </div>
            </div>
             <div class="biaoqian">
...
export default {
  ...
  data () {
    ...
    return {
      ...
      hb:[], // 户表
      pId:'', // 当前选中设备的id
      pName:'', // 当前选中设备的name，用来详细信息里面
      point_name:'', // 当前选中设备的name,用来隐藏和显示
  ...
  methods: {
    ...
    show_DataWindow(radio){
      ...
      this.msg = "{\"client\":\"middle\",\"actionType\":\"show_DataWindow\",\"parameters\":{\"activeName\":\""+this.activeName+"\"}}"
    ...
    sendLine(_line){
     ...
     this.getLineData(_line)
      ...
      document.getElementById('line_name').innerHTML = _line+"接线图"
            //发送mqtt消息
      this.msg = "{\"client\":\"left middle right\",\"actionType\":\"switch_line\",\"parameters\":{\"fline\":\""+_line+"\"}}"
      this.sendMsg(this.msg)
    },
    showLabel(){
    ...
    showDetail(_id,_name){
      ...
      this.msg = "{\"client\":\"left middle \",\"actionType\":\"switch_point\",\"parameters\":{\"pointId\":\""+_id+"\", \"type\":\"detail\",\"dist\":\""+this.dist_graph+"\"}}"
      this.sendMsg(this.msg)
      this.dist_graph = _id
    },
    // 隐藏接线图
    hideDetail(){
      // 如果当前是接线图，则跳到单线图
      if(this.jiexian !=null){
        this.jiexian =null
        this.dist_graph = this.flines[0]
        this.getYZ(this.flines[0])
        this.drawLine(this.flines[0])
        this.msg = "{\"client\":\"left middle right\",\"actionType\":\"switch_line\",\"parameters\":{\"fline\":\""+this.flines[0]+"\"}}"
        this.sendMsg(this.msg)
      }else {  // 如果当前是单线图，跳转到区县
        this.flines = []
        this.dist_graph = this.dist
    ...
    zoomIn () {
      ...
      this.msg = "{\"client\":\"left middle\",\"actionType\":\"switch_zoom\",\"parameters\":{\"action\":\"zoomIn\"}}"
      this.sendMsg(this.msg)
    },
    //缩小
    zoomOut () {
      ...
      if(this.dist === '承德市'){
        ...
        this.msg = "{\"client\":\"left middle\",\"actionType\":\"switch_zoom\",\"parameters\":{\"action\":\"zoomOut\"}}"
      this.sendMsg(this.msg)
    },
    //获取承德市地理图数据
    getCd(){
       ...
       for (let item of this.point_map_bdz) {
        ...
        for(let li of this.line_map_bdz){
         ...
         var line_color,line_style,line_weight
          ...
          var icon = new BMap.Icon('/static/bdmap/images/gt.png', new BMap.Size(21, 21), { fillOpacity: 1, anchor: new BMap.Size(12, 12) })
          var mm = new BMap.Marker(p, { icon: icon ,title:p.name})
           this.gt_guai.push(mm)
      ...
      that.onCd2(this.dist)
        ...
        switch(_dist){
          ...
          case "双滦区":
            ...
            break
          case "高新区":
            var center = new BMap.Point(117.981196,40.920489)
            break
        }
        ...
        for (let item of that.point_d) {
          if(item.dist === that.dist){
            ...
            if(item.type === '变电站'){
              if(item.voltage===35){
                icon = new BMap.Icon('../static/bdmap/images/35.png', new BMap.Size(35, 35), { fillOpacity: 1, anchor: new BMap.Size(18, 18) })
              }else if(item.voltage===110){
                icon = new BMap.Icon('../static/bdmap/images/110.png', new BMap.Size(35, 35), { fillOpacity: 1, anchor: new BMap.Size(18, 18) })
              }else if(item.voltage===220){
                icon = new BMap.Icon('../static/bdmap/images/220.png', new BMap.Size(35, 35), { fillOpacity: 1, anchor: new BMap.Size(18, 18) })
              }else if(item.voltage===500){
                icon = new BMap.Icon('../static/bdmap/images/500.png', new BMap.Size(35, 35), { fillOpacity: 1, anchor: new BMap.Size(18, 18) })
              }
              ...
              switch(item.type){
                ...
                case '杆塔':
                 icon = ''
                  break
                default:
                  icon = new BMap.Icon('../static/bdmap/images/pb.png', new BMap.Size(35, 22), { fillOpacity: 1, anchor: new BMap.Size(13, 11) })
                  break
                  }
            }
  ...
  // 获取线路数据 
    getLineData(_line){
      ...
      post({
            ...
            that.xl =response
            that.chartData1.rows = that.xl.activePowerHis 
            that.chartData2.rows = that.xl.ratedCurrentHis
            that.chartData3.rows = that.xl.currentLoadingHis
        })
    },
   //绘制线路
    drawLine(_line){
                  ...
                  if(po.type === '变电站'){
                      ...
                      switch(po.type){
                        ...
                        icon = new BMap.Icon('/static/bdmap/images/kbz.png', new BMap.Size(35, 35), { fillOpacity: 1, anchor: new BMap.Size(18, 18) })
                          break
                        case '杆塔':
                        icon = new BMap.Icon('/static/bdmap/images/gt.png', new BMap.Size(15, 14), { fillOpacity: 1, anchor: new BMap.Size(13, 7) })
                          break
                        case '熔断器':
                        icon = new BMap.Icon('/static/bdmap/images/rdq.png', new BMap.Size(21, 50), { fillOpacity: 1, anchor: new BMap.Size(11, 25) })
                          break
                        case '配电室':
                        ...
                        icon = new BMap.Icon('/static/bdmap/images/xb.png', new BMap.Size(35, 35), { fillOpacity: 1, anchor: new BMap.Size(18, 18) })
                          break
                        default:
                          icon = new BMap.Icon('/static/bdmap/images/pb.png', new BMap.Size(35, 22), { fillOpacity: 1, anchor: new BMap.Size(20, 13) })
                          break
                          }
                    }
  ...
  // 地理图为主窗口
    change1 () {
      ...
      document.getElementById('map').style.top='171px'
      document.getElementById('map').style.left='104px'
      document.getElementById('map').style.width='1378px'
      document.getElementById('map').style.height='867px'
      document.getElementById('map').style.zIndex=2

      document.getElementById('graphContainer').style.top='736px'
      document.getElementById('graphContainer').style.left='1080px'
    ...
    change2 () {
      ...
      document.getElementById('graphContainer').style.top='171px'
      document.getElementById('graphContainer').style.left='104px'
      document.getElementById('graphContainer').style.width='1378px'
      document.getElementById('graphContainer').style.height='867px'
      document.getElementById('graphContainer').style.zIndex=2

      document.getElementById('map').style.top='736px'
      document.getElementById('map').style.left='1080px'
    ...
    showInfo (e) {
      ...
      // alert(z)   // 11  12 14
          if(this.dist_graph == '承德市'){
        // 14级的时候显示杆塔
            for(let g of this.gt_guai){
                this.map.removeOverlay(g)
                if(z>13){
      ...
      if(e === 2){
        ...
        for (let po of point){
          ...
          if(po.type === '变电站'){
            ...
            }else if(po.type ==='箱式变压器'){
               if (this.checkbox1.indexOf('箱式变压器') >= 0) {
                    this.map.addOverlay(po.marker)
                  po.isShow='true'
                  }
            }else if(po.type ==='电缆分接箱'){
               if (this.checkbox1.indexOf('电缆分接箱') >= 0) {
                    this.map.addOverlay(po.marker)
                  po.isShow='true'
                  }
            }else if(po.type ==='配电室'){
               if (this.checkbox1.indexOf('配电室') >= 0) {
                    this.map.addOverlay(po.marker)
                  po.isShow='true'
                  }
            }
    ...
    main (container) {
      ...
      // 画地图
        ...
        this.graph.setPanning(true)   //// 右键移动容器坐标轴
        var x1,y1
        ...
        //选中图元事件
       var x2,y2,shiftX,shiftY
        this.graph.addListener(mxEvent.CLICK, function(sender, evt) {
    
           x2 = evt.properties.event.offsetX
          ...
          // 点击图元，联动mqtt发消息
          // if(that.graph.getModel().isVertex(that.cell)){
          //     // 如果点击的是节点
          //       // 点联动
          ...
          //   // that.sendPoint(that.pId)
          //   that.onPoint(that.pId)
          //   that.getPoint(that.pId)
          // }
          // if(that.graph.getModel().isEdge(that.cell)){
          //     // 连线
          //         // 线段联动
          //   // 获取线段的起始点坐标
          ...
          if(that.cell.source !==null){
            ...
            that.pId = that.cell.id
            that.pName = that.cell.value
            that.point_name = that.cell.value
            that.onPoint(that.pId)
            that.getPoint(that.pId)
            console.log(that.point_selected)
            // that.cell.setValue('我看看')
            if(that.dist_graph == '滨河新区二路532'){
              if(that.point_name != ''){
                that.point_name = ''
              }else{
                that.point_name= that.point_selected.name
              }
              
              that.graph.getModel().setValue(that.cell, that.point_name);
            }

            //  setValue(that.cell,'我看看')
          }
    ...
    drawYZ(){
      ...
      if(that.flines.length!=0){
       ...
       if(that.jiexian != null){
        document.getElementById('line_name').innerHTML = that.jiexian+'接线图'
        document.getElementById('line_name').style.display = 'block'
      }
     ...
     var count =1
      ...
      for(let li of that.line_wj){
            ...
            var length = li.guai.length
            var p_s =that.graph.insertVertex(that.parent, null, null,li.guai[0].x,li.guai[0].y,1,1)
            var p_t =that.graph.insertVertex(that.parent, null, null,li.guai[length-1].x,li.guai[length-1].y,1,1)
             let el = that.graph.insertEdge(that.parent, li.id, li.fline, p_s, p_t, color)
              var arr =[]
            for (let guai of li.guai) {
              if(guai !=null){
                 arr.push(new mxPoint(guai.x, guai.y))
              }
            }
                el.geometry.points = arr
          }

        for(let item of that.point_wj){
          let labelWidth =item.labelWidth
          let labelHeight = item.labelHeight
          let labelName =item.name
          let string_type ="配变、跌落式熔断器、杆塔、拐点"
          if((that.dist_graph === "滨河新区二路532")&&(string_type.indexOf(item.type) != -1)){
            labelName = ''
          }
          let imgT = new Image()
          if(item.direction !=null){
            that.img='shape=image;image=/static/mx/point/'+item.direction+'_'+item.labelUrl+';'
            imgT.src = '/static/mx/point/'+item.direction+'_'+item.labelUrl

          }else{
            that.img='shape=image;image=/static/mx/point/'+item.labelUrl+';'
            imgT.src = '/static/mx/point/'+item.labelUrl
            if(that.dist === '承德市'){
              that.img='shape=image;image=/static/mx/point/cd_'+item.labelUrl+';'
               imgT.src = '/static/mx/point/cd_'+item.labelUrl
              if(item.voltage ==110){
                labelWidth = 35
                labelHeight =35
              }
          ...
          //     labelWidth = imgT.width
          //     labelHeight = imgT.height
          //      console.log(imgT.src+' '+imgT.width+' '+imgT.height)
          // }
        ...
        let p = that.graph.insertVertex(that.parent, item.id,labelName,item.x-0.5*labelWidth,item.y-0.5*labelHeight,labelWidth,labelHeight,this.img+this.locat)
    ...
    changeSelect(){
     ...
     that.flines =[]
      ...
      that.dist_graph = this.dist
      that.getYZ(that.dist)    // 绘制区县的网架图
      that.getDistData(that.dist) // 绘制区县的地理图
      that.msg = "{\"client\":\"left middle\", \"actionType\":\"switch_dist\", \"parameters\":{\"dist\":\""+this.dist_graph+"\"}}"
      that.sendMsg(that.msg)
   },
  //发送选中设备的命令
    onPoint(value){
      var that = this
              // mqtt 发送消息
       that.msg = "{\"client\":\"left middle right\",\"actionType\":\"switch_point\",\"parameters\":{\"pointId\":\""+value+"\",\"dist\":\""+that.dist_graph+"\"}}"
       that.sendMsg(that.msg)
    },
    //发送选中线路的命令
    onLine(sp,tp){
        var that = this
              // mqtt 发送消息
       that.msg = "{\"client\":\"left right\",\"actionType\":\"switch_line\",\"parameters\":{\"source\":\""+sp+"\", \"target\":\""+tp+"\", \"dist\":\""+that.dist_graph+"\"}}"
       that.sendMsg(that.msg)
    },
    //画承德市内的界限
    onCd0 (url) {
